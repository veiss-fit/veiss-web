        ...
        <PageWrapper>
          {[0, 1, 2].map((index) => (
            <div
              key={index}
              className={`${
                currentSlide === index ? "opacity-1 delay-[400ms]" : "opacity-0"
              } ease-in-out duration-500 transition-opacity absolute left-[1rem] top-[-300px] sm:top-[-400px] lg:top-[-350px] flex flex-col gap-[20px] sm:gap-[30px] lg:gap-[37px] pb-[40px] sm:pb-[60px] lg:pb-[80px]`}
            >
              <div className="p-[5px] flex w-full max-w-[329px] items-center justify-between gap-[10px] bg-white bg-opacity-[10%] rounded-[200px]">
          ...
          <div className="absolute top-[-280px] sm:top-[-330px] right-[1rem] flex flex-row gap-[4px] items-center">
            {[0, 1, 2].map((index) => (
              <div
                key={index}
      ...
      <div className="z-10 relative h-[50%] w-full flex-1 min-h-[300px] sm:min-h-[460px]">
        {[0, 1, 2].map((index) => (
          <div
            key={index}
            className={`${
              currentSlide === index ? "translate-x-0" : " delay-500 translate-x-[-100%]"
